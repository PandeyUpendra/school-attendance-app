...
exports.tempBackfillTeacherIds = onCall(
  { cors: true, region: "asia-south1", enforceAppCheck: false },
  async (request) => {
    const db = admin.firestore();
    ...
    for (const doc of snap.docs) {
      ...
      if (currentTeacherId && currentTeacherId.trim().length > 0) {
        ...
        try {
          ...
          logs.push(`Synced claims for ${email} with existing teacherId ${currentTeacherId}`);
        } catch (e) {
          logs.push(`Could not sync claims for ${email}: ${e.message}`);
        }
        continue;
      }
      ...
      if (!schoolId) {
        errors++;
        logs.push(`Teacher ${email} has no schoolId in allowed_users! Skipping.`);
        continue;
      }
      ...
      try {
        ...
        if (!matchedTeacherDoc) {
          logs.push(`Teacher profile NOT found for email ${email} in school ${schoolId}!`);
          teacherNotFound++;
          continue;
        }

        const resolvedTeacherId = matchedTeacherDoc.id;
        logs.push(`Found teacherId [${resolvedTeacherId}] for ${email}`);
        updated++;
        ...
        try {
          ...
          await auth.setCustomUserClaims(user.uid, claims);
          logs.push(`Successfully updated document and custom claims for ${email}`);
        } catch (claimsErr) {
          logs.push(`Updated document but could not set custom claims for ${email}: ${claimsErr.message}`);
        }

      } catch (err) {
        logs.push(`Error processing teacher ${email}: ${err.message}`);
        errors++;
      }
    }
