...
Login uses **Firebase Auth**. `LoginScreen` signs in with email + password via `AuthService.signInWithEmail()`; guardians use the same email+password flow via `GuardianLoginScreen`, and password reset is handled by `forgot_password_screen.dart`. (Phone-OTP login was removed — the Firestore rules key identity on `request.auth.token.email`, which a phone-auth token lacks, so an emailless session is rejected; re-introducing it requires linking the phone credential to the email account.) After sign-in, the user's role and identifiers are read from the `allowed_users/{email}` document — **keyed by the lowercased email address** (NOT the Firebase Auth UID), holding `role`, `schoolId`, `classIds[]`, `studentIds[]`, etc. The Firestore security rules resolve identity the same way (`getUserData()` reads `allowed_users/$(userEmail())`). `AuthService.saveSession()` then persists the role and identifiers to `SharedPreferences` for fast session routing; `AuthService.clearSession()` signs out of Firebase Auth and clears local state. All services are singletons — construct via the factory `ServiceName()`.
