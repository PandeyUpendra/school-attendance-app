...
class _OwnerHomeState extends State<OwnerHome> {
  ...
  Future<void> _init() async {
    ...
    setState(() {
      ...
      _myRole = session?['role'] as String? ?? 'owner';
  ...
  Future<void> _loadDashboard() async {
    ...
    try {
      ...
      // Active teachers today (those whose class has attendance marked)
  ...
  Future<void> _loadStaff() async {
    ...
    try {
      ...
      final teachers = all
          .where((u) =>
              u['role'] == 'teacher' || u['role'] == 'subjectTeacher')
          .toList();
      final leaves = await _svc.getLeaveApplications(status: 'pending');
  ...
  Future<void> _loadFinance() async {
    ...
    try {
      ...
      final students =
          snap.docs.map((d) => Student.fromJson(Map<String, dynamic>.from(d.data()))).toList();
      ...
      appBar: AppBar(
        ...
        title: Column(
          ...
          children: [
            ...
            Text(_schoolName.isNotEmpty ? _schoolName : _myEmail,
  ...
  Widget _buildBottomNav() {
    return BottomNavigationBar(
      ...
      items: const [
        ...
        BottomNavigationBarItem(icon: Icon(Icons.menu_book_outlined), label: 'Academics'),
        BottomNavigationBarItem(icon: Icon(Icons.currency_rupee_outlined), label: 'Finance'),
  ...
  Widget _buildBody() {
    switch (_tabIndex) {
      ...
      case 2: return _buildAcademicsTab();
      case 3: return _buildFinanceTab();
      case 4: return _buildManageTab();
    ...
    return Padding(
      ...
      child: Column(
        children: filtered.map((t) {
          ...
          final isOnLeave = _pendingLeaves.any((l) =>
              (l['teacherId'] as String? ?? '') == email);
                ...
                Container(
                  ...
                  child: Text(
                    ...
                    style: TextStyle(
                        ...
                        fontWeight: FontWeight.w600,
                        color:
                            isOnLeave ? AppTheme.warning : AppTheme.success),
                  ),
                ),
              ],
            ),
  ...
  Widget _buildUpcomingExams() {
    if (_upcomingExams.isEmpty) {
      return _emptyCard(Icons.event_note_outlined, 'No upcoming exams in the next 30 days');
    }
    return Padding(
      padding: const EdgeInsets.symmetric(horizontal: 14),
      child: Column(
  ...
  Widget _buildCreateAccountForm() {
    final allowed = _perm.getAllowedToCreate(_myRole);
    if (allowed.isEmpty) {
      return _emptyCard(Icons.block_outlined, 'No permission to create accounts');
    }
    return _ownerCard(
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
    ...
    return _ownerCard(
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 10),
          _formField(_schoolAddressCtrl, 'Address', Icons.location_on_outlined),
          const SizedBox(height: 10),
          _formField(_academicYearCtrl, 'Academic Year', Icons.calendar_today_outlined),
          const SizedBox(height: 14),
          SizedBox(
            width: double.infinity,
            child: ElevatedButton.icon(
  ...
  Widget _buildAnnouncementForm() {
    return _ownerCard(
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          _formField(_annTitleCtrl, 'Announcement Title', Icons.title_outlined),
          const SizedBox(height: 10),
          TextField(
  ...
  Widget _buildAnnouncementsList() {
    if (_announcements.isEmpty) {
      return _emptyCard(Icons.announcement_outlined, 'No announcements yet');
    }
    return Column(
